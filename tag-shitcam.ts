...
//
// isKnownShitcam()(pano-meta.ts参照)を使って地点を"Shitcam"としてタグ付けする —
// 既知の低品質サードパーティカバレッジである国/期間の範囲を手動でキュレーションした表。
// resolutionHeight===6656の地点のみをチェックする(READMEのResolutionHeight表を参照)。
// これは限定的で不完全なヒューリスティックであり、表がカバーする特定の国/期間でのみ
// Shitcamを検出できる — 表の範囲外の地点は、実際にShitcamであってもタグ付けされずに
// 残る。これは意図的に許容されているfalse negativeであり、バグではない: この手法が
// 見逃すものは画像ベースのレビューに委ねられる。非Shitcam地点を誤ってShitcamとタグ付け
// することは決してない(表はタグを追加するだけで削除しない)ので、他のタグ付けステップの
// 前後どちらで実行しても安全。
...
async function main() {
  ...
  if (!inputPath || !outputPath) {
    console.error("Usage: npx tsx tag-shitcam.ts <input.json> <output.json> [--only-untagged] [--concurrency=N]");
    process.exit(1);
  }
