 ...
 *
 * getPanoMeta().date(撮影日、"YYYY-M"形式)から撮影月を英語の月名タグ("January"〜"December")として書き込む。
 * 世代を問わずすべての地点が対象(tag-shitcam.tsのresolutionHeightフィルタやtag-watermark-year.tsの世代フィルタとは異なり、
 * 撮影日はどの世代でもGoogleメタデータから直接取得できるため)。
 * 日付が取得できない地点はタグ付けせずスキップする。
 */
...
async function main() {
  ...
  if (!inputPath || !outputPath) {
    console.error(
      'Usage: npx tsx tag-month.ts <input.json> <output.json> [--only-untagged] [--concurrency=N]',
    );
    process.exit(1);
  }
  ...
  await mapConcurrent(entries, concurrency, async ({ c, index }) => {
    ...
    const alreadyHasMonthTag = existingTags.some((t) =>
      (MONTH_NAMES as readonly string[]).includes(t),
    );
