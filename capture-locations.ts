...
/**
 * 使い方:  npx tsx capture-locations.ts <input.json> <outDir> [--only-untagged] [--limit=N] [--concurrency=N]
 *
 * GeoGuessrのcustomCoordinates JSONの各地点について、以下をレンダリングする:
 * - yaw 0°/180°(パノラマ自身の前方/後方方向)での透視投影クロップ(front/back。車が認識しやすい自然な形で写る)、
 * - タイルに焼き込まれた「© YYYY Google」透かしのクロップ(watermark)。
 * 各地点ごとにJPEGと、それらを記述するmanifest.jsonを書き出し、人間(またはClaude)がレビューしてapply-tags.tsでタグ付けできるようにする。
 *
 * 重要: front/backは意図的にpano-meta.tsの`headingDeg`をyawとして使っていない。
 * headingDegは記述的なメタデータ(yaw=0が既にどのコンパス方位を向いているかを教えるだけ)であって、適用すべき回転量ではない。
 * これをyawとして使うと前方から二重に回転してしまう。
 *
 * Googleの公式ビューアをheading=Xでスクリーンショットし、それがyaw=Xではなくyaw=0の出力と一致することを確認済み。
 * 詳細はrender-pano.tsのrenderCarViewsを参照。
 *
 * 透かしの年は撮影日(panoDate)とは別物である点に注意 — 画像が最後に再処理された年を反映しているだけで、より後の年になりうるので、
 * 両者が一致すると仮定しないこと。
 *
 ...
 * 地点も(デフォルト8の)同時実行数の上限付きで処理する。
 * 1000地点あたり、逐次実行なら約6秒/地点(約1.7時間)かかるところを、同時実行に分散させることで同程度の総作業量を大幅に短い時間で終えられる。
 */
...
async function main() {
  ...
  const results = await mapConcurrent(
    coords,
    concurrency,
    async ({ c, index }): Promise<ManifestEntry | null> => {
      ...
      try {
        ...
        console.log(
          `[${index}] saved ${frontFile} (existing tags: ${JSON.stringify(c.extra?.tags ?? [])})`,
        );
        return {
          index,
          panoId,
  ...
  const manifest = results
    .filter((r): r is ManifestEntry => r !== null)
