/**
 * streetlevel(https://github.com/sk-zk/streetlevel)を参考にした、
 * Google Maps内部エンドポイント向けの疑似protobuf URLエンコーダの最小移植版(`!1m2!2sfoo!3d1.5` のような形式を生成する)。
 */
 ...
 * Googleの内部エンドポイントが期待するネストしたオブジェクト/配列構造を素朴にJSのオブジェクトリテラルで表現しているため、
 * フィールドタグ(キー)に意味的な名前は付けられない(相手のprotoスキーマを完全に逆解析しない限り不可能)。
 */
export type PbValue = number | string | boolean | Enum | PbValue[] | PbMessage;
...
function fieldToString(tag: string, value: PbValue): [number, string] {
  ...
  /**
   * `typeof null === "object"` なので、ネストしたメッセージとして扱う前にnull を弾く(元の実装ではここでガードがなく、
   * null値を渡すと`Object.entries(null)` で例外になっていた)。
   */
...
/**
 * Google内部エンドポイントに対してブラウザに見せかけるためのUA。
 * タイルCDN(render-pano.ts)・photometa API(このモジュール)の両方でこの値を使う。
 */
export const UA =
