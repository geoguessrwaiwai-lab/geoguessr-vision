...
残る「Gen2とGen3の区別」は、大半の撮影年では単純な日付比較だけで機械的に決まります。`label-tool/gen2-vs-gen3`でのラベリング結果(1272件)では、2011年だけがGen2/Gen3混在(約52%/48%)で、2009年以前は100%Gen2、2012年以降は100%Gen3という綺麗な境界でした。この境界を`tag-gen2-gen3-by-date.ts`でカットオフとして採用し、2009年以前を`Gen2`、2013年以降を`Gen3`として機械的にタグ付けします。境界年(2010〜2012年、特に2011年)だけが画像から学習したモデル/目視レビューの対象として残ります。

このカットオフはあくまで現時点までにラベリングした地点に基づく経験的な境界なので、ラベルが増えて境界がずれた場合は`tag-gen2-gen3-by-date.ts`内の`GEN2_MAX_YEAR`/`GEN3_MIN_YEAR`を調整してください。なお、このタグ付けは推論対象の地点を機械的に振り分けるだけで、`label-tool/gen2-vs-gen3`のモデル学習データセット自体には影響しません — そちらは撮影年に関わらず全期間のラベルをそのまま学習に使い続けます。

境界年(2010〜2012年)のGen2 or Gen3判定は機械学習によって行います。実装中。
...
- [x] Gen2/Gen3について、撮影年だけで機械的に決まる範囲(2009年以前/2013年以降)は`tag-gen2-gen3-by-date.ts`で画像判定なしにタグ付けするようにしました。境界年(2010〜2012年)だけが画像ベースの判定に残ります。
- [ ] あらかじめ、各国ごとに存在するカメラの世代をファイルに定義しておくことで、画像判定を行わずにタグをより的確に決定できる場合があります(国ごとの撮影時期のばらつきを考慮すると、上記の全世界一律カットオフよりさらに精度を上げられる可能性があります)。
