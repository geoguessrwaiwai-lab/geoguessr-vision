...
まず、Shitcamは「国・撮影年」の組み合わせを用いるだけで機械的に区別できます。つまり、現時点ではShitcamの画質やカメラの特徴を本質的に検出して判断していません。今後新たに新しい国で出現したShitcamなどは一時的に判定を見逃しますが、これは意図的に許容しています。

残る「Gen2とGen3の区別」について説明します。

まず、事前に私が目視で行った`label-tool/gen2-vs-gen3`でのラベリング結果(1941件)では、2011年だけがGen2/Gen3混在(約52%/48%)で、2009年以前は100%Gen2、2012年以降は100%Gen3という綺麗な境界でした。そこで、この境界を信頼し、2009年以前を`Gen2`、2013年以降を`Gen3`として機械的に区別することにしました。
...
さらに、国や地域によってはそもそも「Gen2は存在しない（例:コロンビア）」「Gen3は存在しない（例:マン島）」というケースがあり、このような場合は撮影年度によらず機械的に区別できます。

それでも決まらない地点(2010〜2012年に撮影、かつ、その国・地域にGen2/Gen3両方が存在する場合)だけを、最後に機械学習モデルによって判断する方針としています。
