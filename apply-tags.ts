...
// 使い方:
//   npx tsx apply-tags.ts <input.json> <tags.json> <output.json>
//
// tags.json: [{ "index": 0, "panoId": "...", "tags": ["Blue 24"] }, ...]
// (capture-locations.tsが出力したレンダリング結果をレビューして作成する)
//
// 各エントリのタグをcustomCoordinates[index].extra.tagsにマージする。既に存在する
// (大文字小文字を無視して比較する)タグはスキップするので、再実行しても安全。panoIdは
// 入力元ファイルと突き合わせて検証し、古い/対応がずれたタグファイルを検出する。
