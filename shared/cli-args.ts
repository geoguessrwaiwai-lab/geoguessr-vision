...
//
// 各スクリプトが `args.find(a => a.startsWith("--limit="))` のような手書きパースを
// 個別に実装していた(capture-locations.ts, tag-watermark-year.ts,
// label-tool/capture-for-labeling.ts で3箇所以上重複)。Node組み込みの
// `node:util.parseArgs` は事前にオプション名を列挙する必要があり、本ツール群のように
// 「位置引数+`--flag=value`」が混在する簡易なCLIにはやや大仰なため、既存の挙動を
// 保ったまま関数として共通化するに留める。
