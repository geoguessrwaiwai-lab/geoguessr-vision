...
// 使い方:
//   npx tsx tag-gen2-gen3-by-model.ts <input.json> <output.json> [--only-untagged]
//     [--concurrency=N] [--min-confidence=0.7] [--model=models/gen2-vs-gen3.onnx]
//
// tag-gen2-gen3-by-date.tsが機械的に振り分けられない境界年(2010-2012年)の地点を、
// training/gen2-vs-gen3/で学習したONNXモデルで判定する。resolutionHeight===6656
// (Gen2/Gen3/Shitcamの可能性がある地点)のうち、既にGen2/Gen3/Shitcamタグを持つ地点や
// 既知のShitcam(tag-shitcam.ts参照)は対象外。
//
// 重要: ここでのfront/backレンダリングは、training/gen2-vs-gen3/で実際に学習した
// label-tool/gen2-vs-gen3/images/*/front.jpg・back.jpgと同じ幾何(yaw 0°/180°、
// pitch -20°、fov 80°、900x700)でなければならない — renderCarViews()のデフォルト値が
// これに一致する。レンダリングの幾何を変更する場合は、学習データセットの再レンダリング・
// モデルの再学習とセットで行い、ここも追随させること。
//
// 前処理(リサイズ・正規化)はtraining/gen2-vs-gen3/dataset.pyのeval変換
// (Resize((224,224)) + ImageNet正規化)と一致させている。
...
async function toModelTensor(image: Sharp): Promise<ort.Tensor> {
  ...
  // sharpのrawバッファはHWC(インターリーブRGB)のuint8。モデルはCHWのfloat32を
  // 期待するため、正規化しつつチャンネルごとの平面に並べ替える。
...
async function main() {
  ...
  const modelPath = getFlagValue(args, "model") ?? path.join(import.meta.dirname, "models", "gen2-vs-gen3.onnx");
  ...
  if (!fs.existsSync(modelPath)) {
    console.error(`Model not found at ${modelPath}. Run training/gen2-vs-gen3/export_onnx.py first.`);
    process.exit(1);
  }
  ...
  await mapConcurrent(entries, concurrency, async ({ c, index }) => {
    ...
    try {
      ...
      console.log(`[${index}] ${panoId} (${meta.countryCode} ${meta.date}) -> "${gen}" confidence=${confidence.toFixed(3)}`);
    } catch (e) {
