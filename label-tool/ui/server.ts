 ...
 * ラベルは<dataDir>/labels.jsonにpanoIdをキーとして永続化されるので、ツールを閉じて再度開いても進捗は失われない。
 *
 * <dataDir>/model.json があれば、そのモデル専用の設定(選択可能な世代・タイトルなど)として読み込む。
 * モデルごとにデータフォルダを分ける(例: gen2-vs-gen3/)ことで、ラベリングUI・サーバーのコード自体は1つのまま、
 * モデルごとに選択肢だけを変えられる。
...
/**
 * import.meta.dirnameはNode 20.11+/21.2+で使える比較的新しいAPI。
 * tsxはホストのNodeランタイム上で動くため、これより古いNodeでは動作しない点に注意。
...
const config: LabelToolConfig = fs.existsSync(configPath)
  ...
  : {
      name: path.basename(path.resolve(dataDir)),
      generations: [...GENERATIONS],
      colorGens: [...DEFAULT_COLOR_GENS],
    };
  ...
  if (parsed.pathname === '/api/label' && req.method === 'POST') {
    ...
    req.on('end', () => {
      ...
      try {
        ...
        /**
         * 元の実装はここでJSON.parseの例外を捕捉しておらず、不正なリクエストボディを送るとこのリクエストハンドラがクラッシュしていた(プロセス全体は落
         * ちないが、レスポンスも返らずクライアントがハングする)。
         * 400を返して安全に終える。
         */
