 ...
 * panoIdごとにgetPanoMeta()(Googleのphotometaエンドポイント)へ問い合わせて取得し直す(capture-locations.tsのコメント参照:
 * 表示用headingとyawは別物)。
 *
 * デフォルトでは resolutionHeight===6656(Gen2/Gen3/Shitcamの可能性がある地点)だけを残す。
 * Gen1/Gen4も含めた全世代を集めたい場合は --all-resolutions を付ける。
 *
 * resolutionHeight===6656に絞り込む場合、isKnownShitcam()(pano-meta.ts参照、
 * tag-shitcam.tsと同じ既知の国/期間テーブル)に一致する地点はGen2 vs Gen3の候補から除外する(AGENTS.mdのモデル分割方針: Shitcamは対象外、
 * ラベリング労力の無駄を避ける)。
...
async function main() {
  ...
  const results = await mapConcurrent(
    withPanoId,
    concurrency,
    async (loc, i): Promise<Candidate | null> => {
      try {
        ...
        if (!allResolutions && isKnownShitcam(meta)) {
          console.log(
            `[${i + 1}/${withPanoId.length}] ${loc.panoId} known Shitcam (${meta.countryCode} ${meta.date}), skipping`,
          );
          return null;
        }
        console.log(
          `[${i + 1}/${withPanoId.length}] ${loc.panoId} resolutionHeight=${meta.resolutionHeight}`,
        );
        return {
          panoId: loc.panoId,
        ...
        console.log(
          `[${i + 1}/${withPanoId.length}] ${loc.panoId} FAILED: ${(e as Error).message}`,
        );
        return null;
      }
    },
