 ...
 *
 * ラベリングUIで棄却(Xキー)された地点を、labels.json・items.json・候補プール(candidates/*.json)・images/から取り除く。
 */
...
function main() {
  ...
  if (!dataDirArg) {
    console.error(
      'Usage: npx tsx label-tool/migrate-label-format.ts <dataDir> [candidate.json ...]',
    );
    process.exit(1);
  }
  ...
  for (const panoId of rejectedIds) {
    ...
    /**
     * 削除前に、実際にimagesRoot配下のパスであることを確認する安全チェック。
     * panoIdはlabels.jsonのキー(通常は内部生成された英数字ID)なので実運用で問題になることは想定していないが、
     * `fs.rmSync(..., { recursive: true })`という再帰削除の操作の重大さに対して、この程度のガードは費用対効果が高い。
     */
