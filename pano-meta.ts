...
// パノラマの真のカメラ向き(photometa)を、Google Maps自身のWebサイトが使っているのと
// 同じ内部エンドポイントから取得する。
...
export function classifyResolutionHeight(
...
// Street View左下の著作権表記は、公式Google撮影なら「© 2024 Google」、サードパーティ
// (トレッカー・行政機関等)撮影なら「© <年> <会社/機関名>」になる。年プレフィックスを
// 取り除いた残りが厳密に"Google"と一致する場合だけ公式とみなす。
...
// これは手動でキュレーションされた既知の国/期間の組み合わせのリストであり、汎用的な検出器ではない
// このリストから漏れる範囲では、ShitcamをShitcamとして検出できないが許容する。
...
export function isKnownShitcam({
  date,
  countryCode,
  lat,
}: IsKnownShitcamInput): boolean {
